...
<div style="margin-top: 5%; margin-left:30%; padding:1px 16px;height:1000px;">
	...
	if (isset($_POST['submit'])) {
		...
		$sql = "SELECT * FROM `role`
		WHERE LOWER(nama_role) LIKE '%$cari_menu%'
		OR id_role like '%$cari_menu%';";
	...
	<table class=" table table-bordered table-striped table-hover" style="margin-left:0%; padding:1px 16px; width: 800px; ">
		...
		<thead align="center">
		<tr>
			<th style="background-color: #2293df; color: #111;">ID Role</th>
			<th style="background-color: #2293df; color: #111;">Nama Role</th>
		...
		<?php foreach ($role as $row) : ?>
			...
			<td align="center">
				<a href="ubah-role.php?id=<?=$row['id_role']?>" class="btn btn-warning"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-fill" viewBox="0 0 16 16">
				  <path d="M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708l-3-3zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207l6.5-6.5zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.499.499 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11l.178-.178z"/>
				</svg> Edit</a> 
				<a href="hapus-role.php?id=<?=$row['id_role']?>"  class="btn btn-danger">
